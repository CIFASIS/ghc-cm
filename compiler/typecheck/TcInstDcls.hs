...
tcClsInstDecl (L loc (ClsInstDecl { cid_poly_ty = hs_ty, cid_binds = binds
  ...
  = setSrcSpan loc                      $
    ...
    do  { traceTc "tcLocalInstDecl" (ppr hs_ty)
        ...
        ; let (tyvars, theta0, clas, inst_tys) = tcSplitDFunTy dfun_ty
...
mkBind (ClsInstMorph { info_cls = cls_inst
                     , info_morph = Just m
                     , ant_dfun = Just dfun_a}) =
    do
       --
       -- Say we have
       -- [...]
       -- class D a => C a
       -- di :: instance A a => B [a]
       -- dm :: class morphism B -> C
       --
       -- Then we generated
       -- dm<di> :: instance A a => C [a]
       --
       -- Remember that dm is really a function of type:
       -- forall a. C a -> B a -> D a -> C a
       -- First argument should be tied recursively, then comes the
       -- antecedent dictionary and then the superclasses of the class
       -- we're building (D a, but possibly more), which should be
       -- generated by the simplifier.
       --
       -- So, we need to bind `dfun_c` to
       -- dfun_c = /\a. \(dict_a : DictA a). dm {[a]} (dfun_c {a} dict_a) (di {a} dict_a) _
       --
       -- dfun_c = /\a. \(dict_a : DictA a).
       --        let knot : DictC [a] = dm {[a]} knot (di {a} dict_a) _ in knot
       --
       -- where _ is to be filled by the simplifier
       --
       -- NB: dm {[a]} has type C [a] -> B [a] -> D [a] -> C [a]
       --
       let dfun_c = is_dfun cls_inst
       (inst_tyvars, dfun_theta, inst_head) <- tcSkolDFunType dfun_c
       let (clas, inst_tys) = tcSplitDFunHead inst_head
       dfun_ev_vars <- newEvVars dfun_theta

       let (class_tyvars, sc_theta, _, _) = classBigSig clas
       let sc_theta' = substTheta (zipTvSubst class_tyvars inst_tys) sc_theta
       ...
       uniq <- newUnique
       self_dict <- newDict clas inst_tys
       let oc_name = mkVarOcc "knot_morph"
           knot_name = mkInternalName uniq oc_name noSrcSpan
           knot_v = mkLocalId knot_name inst_head
           knot_t = HsVar noExt (noLoc knot_v)
           letdef0 = mkHsApps (noLoc morph_inst) (map noLoc $ [knot_t, ant_app])
           letdef = noLoc $ mkHsWrap (mkWpEvVarApps sc_ids) (unLoc letdef0)
           letbody = knot_t
           letbindings = noLoc $ HsValBinds noExt $ XValBindsLR $
                            NValBinds [(Recursive, unitBag $ mkVarBind knot_v letdef)] []
           letexpr = noLoc $ HsLet noExt letbindings $ noLoc letbody
       -------
           morph_inst = mkHsWrap (mkWpTyApps inst_tys) morph
           morph = HsVar noExt (noLoc (mDFun m))
           ant =  HsVar noExt (noLoc dfun_a)
           ant_inst = mkHsWrap (mkWpTyApps $ map mkTyVarTy inst_tyvars) ant
           ant_app = mkHsWrap (mkWpEvVarApps dfun_ev_vars) ant_inst

       let dict_bind = mkVarBind self_dict letexpr
       traceTc "mkBind, generated binds" $ ppr dict_bind
...
mkBind i = pprPanic "Unexpected ClsInstMorph" $ ppr i
